...
// check if there are any lazy image elements on the page
const $lazyImages = jquery('.g-freebird-lazy');
if ($lazyImages.length > 0) {

  const getPlatform = function () {
    return isMobile() || getViewport().width < 400 ? 'Mobile' : 'Desktop';
  };

  const setRatio = function ($el) {
    const data = $el.data();
    const platform = getPlatform();

    // 1)
    // because we may need to revert to a non-cropped image from a cropped image
    // on later resizes, keep a copy of the source image aspect ratio.
    // If it does exist, revert to it before we recalculate needed ratio for new source image.
    if (!data._ratio) {
      data._ratio = data.ratio; // cache original
    } else {
      data.ratio = data._ratio; // revert from cache
    }

    // 2)
    // update `data.ratio` to take into account
    // platform specific crops. This will use
    // the original value, restored from cached
    // above, if no platform crop is needed;
    data.ratio = data[`crop${platform}Ratio`] ? data[`crop${platform}Ratio`] : data.ratio;

    if(isScoopImage($el) && data['mobileCrop'] && getViewport().width <= 1024 && isPortrait()){
      data.ratio = 1.5;
    }

  };

  //hacked in responsive scoop images//checking if scoop image by data-pattern
  const isScoopImage = function($el){
    const data = $el.data();
    const pattern = data.pattern;
    return pattern && pattern.indexOf("{{file}}") >= 0;
  };


  $lazyImages.each(function () {

    // because laziestloader isn't aware of our multiple platform crops and
    // the various ratios they could be, we need to manually set the
    // `data.ratio` property before running laziestloader.
    setRatio(jquery(this));

    // now bind laziestloader
  }).laziestloader({
    threshold: getViewport().height * 2,
    getSource: function ($el) {

      const data = $el.data();
      const $parent = $el.parent().parent();
      let width = (data.ratio < 1) ? $el.height()/data.ratio : $el.width();


      //hacked in responsive scoop images//taken from rad-images.js from vi RAD
      if(isScoopImage($el)){
        var filename = null;
        var widths = data.widths.master;

        if (getViewport().width <= 1024 && data['mobileCrop'] && isPortrait()) {
          widths = data.widths.mobileMaster;
        }

        filename = (function () {
          var sizes = jquery.map(widths, function (val) { return val.size; });
          var bestFitWidth = jquery().laziestloader.bestFit(width, sizes);
          for (var i = widths.length - 1; i >= 0; i--) {
            if (widths[i].size === bestFitWidth) {
              return widths[i].filename;
            }
          }
        })();
        //console.log(filename);
        return data.pattern.replace(/{{file}}/ig, filename);
        ...
        // construct data-pattern for responsive renditions.
        // prefer a platform crop over the default, uncropped image if needed.
        const pattern = data[`crop${platform}Pattern`] ? data[`crop${platform}Pattern`] : data.pattern;
        const patternRetina = data[`crop${platform}PatternRetina`] ? data[`crop${platform}PatternRetina`] : data.patternRetina;

        // for path to image, prefer cropped, responsive renditions (via data-pattern)
        // but fall back to non-responsive data-src if needed.
        return (function(){
          let sourcePath = '';

          if(pattern){

            // use retina if needed and possible
            sourcePath = retina && patternRetina ? patternRetina : pattern;

            // substitute {{size}} in url path with the best sized rendition
            sourcePath = sourcePath.replace('{{size}}', jquery().laziestloader.bestFit(width, data.widths));
          } else {
            sourcePath = data.src;
          }
    ...
    // recheck that `data.ratio` is correct after
    // all source updates
    setRatio($el);

    // scollslides, among other apps, need to know when an image
    // has been updated, as it may have changed the size of the DOM
    $el.trigger('freebird:lazyimage');
  });
}
...
//import scrollingVideo from './scripts/freebird/scrollingvideo.js';
...
const create = (el) => {
  let throttled = false;
  let scrollPct = 0;
  let winWidth = Math.max(document.documentElement.clientWidth, window.innerWidth);
  let winHeight = Math.max(document.documentElement.clientHeight, window.innerHeight);
  let scrollTextWidth, video;


  const scrollContainer = el,
    mediaContainer = scrollContainer.querySelector('.g-media'),
    innerContainer = mediaContainer.querySelector('.g-media-inner'),
    isVideo = scrollContainer.classList.contains('g-slide-video'),
    scrollText = (isVideo) ? mediaContainer.querySelector('.g-slide-scrolltext') : undefined,
    scrollTextInner = (isVideo) ? scrollText.querySelector('.g-slide-scrolltext-inner') : undefined,
    prevContainer = scrollContainer.previousElementSibling,
    isLede = (prevContainer && prevContainer.classList.contains('g-lede')),
  ...
  let navContainer = (isLede) ? (winWidth < 1024) ? prevContainer.querySelector('.g-mobile').querySelector('.g-top') : prevContainer.querySelector('.g-desktop').querySelector('.g-top') : undefined;
  let containerHeight = (isVideo) ? winHeight * 2.2 : (isLede) ? winHeight * 1.9 : winHeight;

  //scrollContainer.style.height = `${containerHeight}px`;

  if(isVideo){
    scrollTextWidth =  scrollTextInner.getBoundingClientRect().width + winWidth * 1.1;
    scrollText.style.width = `${scrollTextWidth}px`;
    const videoId =   mediaContainer.querySelector('.g-vhs').id;
    video = players.find(p => p.id === videoId)
    if(videoId.indexOf('cuties') >= 0 && video && video.player){
  ...
  {
    winWidth = Math.max(document.documentElement.clientWidth, window.innerWidth);
    winHeight = Math.max(document.documentElement.clientHeight, window.innerHeight);

    if (!throttled) {
      navContainer = (isLede) ? (winWidth < 1024) ? prevContainer.querySelector('.g-mobile').querySelector('.g-top') : prevContainer.querySelector('.g-desktop').querySelector('.g-top') : undefined;
      containerHeight = (isVideo) ? winHeight * 2.2 : (isLede) ? winHeight * 1.9 : winHeight;
      scrollContainer.style.height = containerHeight;

      if(isVideo){
        scrollText.style.width = '1000vw';
        scrollTextWidth =  scrollTextInner.getBoundingClientRect().width + winWidth * 1.1;
        scrollText.style.width = `${scrollTextWidth}px`;
      }

      scene.offset(-winHeight);
      scene.duration(containerHeight);
      throttled = true;
    ...
    scene.addTo (controller)
      ...
      .on ("progress", (e) => {
        ...
        if (scrollPct !== e.progress) {
          ...
          scrollPct = scrollAdj;
          let diff = (isVideo) ? (containerHeight/winHeight * 1.5) : 1;
          mediaContainer.style.height = `${ (scrollPct * diff) * 100}vh`;
          innerContainer.style.top = `${ -(100 - (scrollPct * diff * 100))}vh`;
          if(isVideo){
            let pct = 0.4;
            if(scrollPct > pct ){
              const p = (scrollPct - pct)/(1 - pct);
              scrollText.style.transform = `translate3d(${ -( p) * 100}%, 0,0)`;
            }
          }
          if(isLede) navContainer.style.opacity = 1-(scrollPct * diff);

        }
        //console.log ("scrollingslide", "scrollPct", scrollPct);
      })
      .on("enter", (e) => {
        if(isVideo && video && video.player) video.player.play();
        if(isLede) prevContainer.classList.add('g-past');
      })
      .on("leave", (e) => {
        if(isVideo && video && video.player) video.player.pause();
        if(isLede) prevContainer.classList.remove('g-past');
      });

  };
  startScrollAnimation();
};
const story = document.querySelector('.g-story');
const items = [...story.querySelectorAll('.g-slide')];
items.forEach((item,index) => {
  if(!item.classList.contains('g-lede')){
    create(item);
  }

});

var headerDates = [...document.querySelectorAll(".g-slide-date")];
var date = document.querySelector(".interactive-timestamp");
if (date) {

  headerDates.forEach((d,index) => {
    // console.log(d);
    d.appendChild(date.cloneNode(true));
  });
